...
def get_video_transcripts(videos: list):
    """Retrieve text transcripts for the top 3 YouTube videos."""
    combined_transcripts = []
    ytt = YouTubeTranscriptApi()
    
    for i, vid in enumerate(videos, start=1):
        try:
            # ytt.fetch() retrieves snippet objects containing .text
...
def generate_ai_editorial(problem_title: str, platform: str, difficulty: str):
    """
    1. Search YouTube for top 3 solution videos.
    2. Extract transcripts using YouTubeTranscriptApi().fetch().
    3. Use GEMINI_EDITORIAL_API_KEY (if valid) or fallback to GEMINI_API_KEY to generate editorial.
    """
